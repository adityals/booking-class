...
/** Every column of the row types below, so the SQL never repeats an eight-item list. */
const BOOKING_COLUMNS = `id, student_id, trial_class_id, status, amount_cents,
        held_at, created_at, updated_at`;
const ATTEMPT_COLUMNS = `id, booking_id, status, amount_cents, idempotency_key,
        provider_ref, error, created_at, settled_at`;
  ...
  async preparePayment(bookingId: number): Promise<PreparedPayment> {
    return withTransaction(this.pool, async (tx) => {
      ...
      if (!claimed) {
        ...
        return {
          kind: "unavailable",
          booking: mapBooking(unavailable.rows[0]),
          attempt: null,
        };
      }
